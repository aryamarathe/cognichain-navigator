      ...
      <header className="border-b border-border/60 backdrop-blur-md sticky top-0 z-50 bg-background/70">
        <div className="max-w-[1600px] mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1 text-sm">
            {(["Overview", "Logistics", "Inventory", "Forecast", "Simulation"] as const).map((n) => (
              <button
                key={n}
                onClick={() => setView(n)}
                className={`px-3 py-1.5 rounded-md transition-colors ${view === n ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted/40"}`}
              >
                {n}
              </button>
            ))}
